...
description: >-
  Step 12 of HYPERRESEARCH V8 — four adversarial critics (dialectic, depth, width, instruction) run in parallel and emit findings JSONs.
...
#                 context="vault_tag=<vault_tag>; from step hyperresearch-<critic-name>-critic subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-<critic-name>-critic`
   prompt: |
...
```
skill_view(name="hyperresearch-13-gap-fetch")
```
