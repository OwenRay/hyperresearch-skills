...
description: >-
  Step 10 of HYPERRESEARCH V8 — per-angle source curation then 3 parallel draft-orchestrators produce angle-specific drafts.
...
5. **Exit and route.** Once `research/notes/final_report_<vault_tag>.md` is written, return to the entry skill and invoke `skill_view(name="hyperresearch-15-polish")`. Light tier skips steps 11–14 entirely.
...
> **Hermes translation:** the Claude-Code `Task(...)` spawn described below maps to a Hermes `delegate_task(role=\"leaf\", goal=\"<rendered prompt block>\", context=\"vault_tag=<vault_tag>\")`. Issue one `delegate_task` call per subagent, together in one turn for parallelism.
...
#                 context="vault_tag=<vault_tag>; from step hyperresearch-draft-orchestrator subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-draft-orchestrator`
prompt: |
...
- **light tier:** You already wrote `research/notes/final_report_<vault_tag>.md` directly. Skip steps 11-14 (no synthesis, no critics, no patcher) and invoke `skill_view(name="hyperresearch-15-polish")`.
- **full tier:** Invoke `skill_view(name="hyperresearch-11-synthesize")`.
