...
description: >-
  Step 2 of HYPERRESEARCH V8 — build a multi-perspective search plan and run source-count-gated parallel fetcher waves until the source_target funnel is met.
...
```
# Hermes port: this Claude-Code Task spawn becomes
#   delegate_task(role="leaf",
#                 goal="""<the rendered prompt block that follows this line>""",
#                 context="vault_tag=<vault_tag>; from step hyperresearch-fetcher subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-fetcher`
prompt: |
  RESEARCH QUERY (verbatim, gospel):
  > [PASTE the verbatim contents of research/runs/<vault_tag>/query.md here]
  ...
  FETCH METHOD (Hermes escalation lane): your default fetcher is `$HPR fetch <url>`.
  If a URL returns a shell/empty page, a 403/"Unauthorized", a bot-wall, or is a
  JS-rendered SPA, escalate to the remote Mac's REAL browser — load the skill
  `hyperresearch-browser-fetch` (skill_view) and follow its recipe: launch a
  headless Chrome on $REMOTE_HOST over an SSH tunnel, drive it with
  chrome-remote-interface, save the rendered HTML to
  research/runs/<vault_tag>/notes/<slug>.html, then `$HPR note add` it as a source.
  Prefer `$HPR fetch` for ordinary pages (it captures metadata + quotes
  automatically); use the browser only when that fails.
```
...
#   delegate_task(role="leaf",
#                 goal="""<the rendered prompt block that follows this line>""",
#                 context="vault_tag=<vault_tag>; from step hyperresearch-browser-fetcher subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-browser-fetcher`
prompt: |
  RESEARCH QUERY (verbatim, gospel):
  > [PASTE the verbatim body of research/runs/<vault_tag>/query.md here]
...
#   delegate_task(role="leaf",
#                 goal="""<the rendered prompt block that follows this line>""",
#                 context="vault_tag=<vault_tag>; from step hyperresearch-source-analyst subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-source-analyst`
prompt: |
  RESEARCH QUERY (verbatim, gospel):
  > [PASTE the verbatim body of research/runs/<vault_tag>/query.md here]
...
- **light tier:** Skip directly to step 10 — invoke `skill_view(name="hyperresearch-10-triple-draft")` (light tier writes a single draft, not the ensemble)
- **full tier:** Invoke `skill_view(name="hyperresearch-3-contradiction-graph")`
