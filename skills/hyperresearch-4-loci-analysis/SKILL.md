...
description: >-
  Step 4 of HYPERRESEARCH V8 — two loci-analysts (sequential) score candidate loci and emit loci.json with per-source reading budgets.
...
#                 context="vault_tag=<vault_tag>; from step hyperresearch-loci-analyst subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-loci-analyst`
   prompt: |
...
```
skill_view(name="hyperresearch-5-depth-investigation")
```
