...
description: >-
  Step 1.5 (dissertation tier only) — partition atomic items into 4–10 chapters; steps 2–10 then loop per chapter within the gear envelope.
...
**Profile gate:** Runs ONLY when the resolved profile's `chapters` range is non-zero (e.g. `dissertation` = 4–10 chapters). For `light` and `full`, this step does not exist — skip directly to step 2.

**Why chapters:** no single agent can hold a 300–450-source corpus. Chapters make the proven 40–80-source pipeline the *unit of execution*: each chapter runs its own width sweep through drafting, and global layers integrate on top. The ~80-source quality ceiling is per-chapter, not per-run.
...
1. **Decide chapter count.** Group the decomposition's atomic items into 4–10 topically cohesive chapters. Rules:
   ...
   - A chapter is a *research unit*: it must be answerable with 40–80 sources. If a candidate chapter obviously needs more, split it.
...
For each chapter (respecting `depends_on`, up to 2 chapters in flight):
...
Return to the entry skill and begin the chapter execution loop at step 2 for the first chapter(s): `skill_view(name="hyperresearch-2-width-sweep")`.
