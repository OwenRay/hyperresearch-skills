...
description: >-
  Step 5 of HYPERRESEARCH V8 — K depth-investigators run in parallel to produce interim notes that commit to a position on their locus.
...
#                 context="vault_tag=<vault_tag>; from step hyperresearch-depth-investigator subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-depth-investigator`
   prompt: |
...
```
skill_view(name="hyperresearch-6-cross-locus-reconcile")
```
