...
description: >-
  Step 8 of HYPERRESEARCH V8 — 'what source would overturn this?' audit plus a targeted gap-fill fetch wave.
...
#                 context="vault_tag=<vault_tag>; from step hyperresearch-corpus-critic subagent>")
# Spawn one delegate_task per subagent; issue them in a single turn for parallelism.
# Original subagent_type was: `hyperresearch-corpus-critic`
   prompt: |
...
# Original subagent_type was: `hyperresearch-fetcher`
   prompt: |
     ...
     FETCH METHOD (Hermes escalation lane): default = `$HPR fetch <url>`.
     If a target is a JS-rendered SPA, behind a bot-wall, or returns 403/"Unauthorized",
     escalate to the remote-Mac real browser via the `hyperresearch-browser-fetch` skill
     (launch headless Chrome on $REMOTE_HOST over an SSH tunnel, drive with
     chrome-remote-interface, save rendered HTML as a source note).
```
...
```
skill_view(name="hyperresearch-9-evidence-digest")
```
